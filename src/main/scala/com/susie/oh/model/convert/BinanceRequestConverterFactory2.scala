  ...
  override def getResponse(orderBookRequest: OrderBookRequest, httpResponse: HttpResponse)(implicit ec: ExecutionContext, mat: ActorMaterializer): Future[(Price, Price)] = {
    ...
    httpResponse.status match {
      ...
      case StatusCodes.OK => {
        ...
        val firstPrice = Price(orderBookRequest.leg, orderBookRequest.requestFactory.exchangeProfile.id, highestBid * (1 + orderBookRequest.requestFactory.exchangeProfile.limitFee), highestBidVolume)
        
        val secondVolumeConverted = lowestAskVolume * lowestAsk
        
        val secondPrice = Price(orderBookRequest.leg.swap(), orderBookRequest.requestFactory.exchangeProfile.id, (1 / lowestAsk) * (1 + orderBookRequest.requestFactory.exchangeProfile.limitFee), secondVolumeConverted, true)
